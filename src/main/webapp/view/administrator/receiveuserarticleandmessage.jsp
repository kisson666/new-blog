...
<head>
    ...
    <title>管理员查看指定用户的指定一篇文章</title>
</head>
<body>
<center>
    <font color="red">文章内容</font><br>
    <% out.println(request.getAttribute("content"));%>
    <form action="deleteuserarticle" method="post">
    ...
    <br><br>
    <font color="red">留言</font>&nbsp;&nbsp;<a href="#writemessage">点击给TA的文章留言</a><br>
    <br>
    <%List<Message> messages = (List<Message>) request.getAttribute("messages"); %>
    <%!long i = 1; %>
    <%if (messages.isEmpty()) { %>
    <font color="red"><%out.println("暂时还没有人留言");%></font><br>
    <%} else { %>
    <% for (Message message : messages) {%>
    <font color="red"><%out.println(i + "楼  ");%></font>
    <font color="red">留言内容:</font><%out.println(message.content + "  ");%>
    <font color="red">楼主的ID:</font><%out.println(message.id + "  ");%>
    <font color="red">楼主的昵称:</font><%out.println(message.name + "  ");%>
    <font color="red">留言时间:</font><%out.println(message.time + "  ");%>
    ...
    <%} %>
    <br><br>
    <font color="red">给TA的文章留言</font><br>
    <a name="writemessage"></a>
    <form action="administratoraddmessage" method="post">
        <!-- 加载编辑器的容器 -->
